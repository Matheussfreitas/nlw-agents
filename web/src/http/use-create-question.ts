...
export function useCreateQuestion(roomId: string) {
  ...
  return useMutation({
    ...
    onMutate({ question }) {
      const questions = queryClient.getQueryData<GetQuestionsResponse>(['get-questions', roomId])
    ...
    onSuccess(data, _variables, context) {
      queryClient.setQueryData<GetQuestionsResponse>(
        ['get-questions', roomId],
        (questions: GetQuestionsResponse | undefined): GetQuestionsResponse | undefined => {
          if (!questions) {
            return questions;
          }
    ...
    onError(_error, _variables, context) {
      if (context?.questions) {
        queryClient.setQueryData<GetQuestionsResponse>(['get-questions', roomId], context.questions);
      }
    },
  })
}